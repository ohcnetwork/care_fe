    ...
    return (
        <div className="w-full relative">
            <div className="flex gap-1 flex-wrap my-2">
                {selectedValues.length > 0 ? selectedValues.map((selectedValue, i)=>{
                    return (
                        <div
                            key={i}
                        ...
                        >
                            ...
                            <button
                                type="button"
                                onClick={()=>{
                                    setSelectedValues(selectedValues.filter((v)=>v!==selectedValue))
                                }}
                            >
            ...
            >
                {
                    options.filter(o=>o.toLowerCase().includes(value.toLowerCase())).map((option, i)=>{
                        return (
                            <button
                                type="button"
                                key={i}
                                className={clsx([
                                    ...
                                    ], {"bg-primary-100 hover:bg-primary-200" : selectedValues.includes(option)}
                                )}
                                onClick={()=>{
                                    setSelectedValues(selectedValues.includes(option) ? selectedValues.filter(v=>v!==option) : [...selectedValues, option])
                                }}
                            >
                                {option}
                            </button>
