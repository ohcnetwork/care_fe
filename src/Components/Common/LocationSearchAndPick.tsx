...
export const LocationSearchAndPick = (props: LocationSearchAndPickProps) => {
    ...
    return (
      ...
      >
        <div className={classes.locationResults}>
          {isLocationSearchInProgress ? (
            <div className={classes.placeholder}>Loading...</div>
          ) : locationSearchResults.length === 0 ? (
            <div className={classes.placeholder}>No Results </div>
          ) : (
  ...
  const circleMode = {
    ...
    control: {
      ...
      values: [
        { center: [latitude, longitude], radius: DEFAULT_MARKER_RADIUS },
      ],
    },
  };

  return (
    <div key={`${latitude}_${longitude}`}>
      <TextInputField
