...
const ProfilePicUploadModal = ({
  ...
  const handleUpload = async () => {
    ...
    const url = `/api/v1/users/${user.username}/profile_picture/`;
    setIsUploading(true);
  ...
  const hasImage = !!(preview || user.read_profile_picture_url);
  const imgSrc =
    preview || `${user.read_profile_picture_url}?requested_on=${Date.now()}`;
  ...
  const commonHint = (
    <>
      ...
      {t("recommended_aspect_ratio_for")} user profile image is 1:1
    </>
  );
