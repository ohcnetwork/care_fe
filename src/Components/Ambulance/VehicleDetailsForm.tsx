...
export const VehicleDetailsForm = (props: any) => {
	...
	const [form, setForm] = useState<any>(
		Object.assign(initForm, vehicleDetails)
	);
	...
	const handleChange = (e: any) => {
		...
		if (
			...
			setDistrictOptions({
				primaryDistrict: allDistrictOptions.filter(
					i =>
						!i.id ||
						!(
							i.id === fieldValue.secondaryDistrict ||
							i.id === fieldValue.thirdDistrict
						)
				),
				secondaryDistrict: allDistrictOptions.filter(
					i =>
						!i.id ||
						!(
							i.id === fieldValue.primaryDistrict ||
							i.id === fieldValue.thirdDistrict
						)
				),
				thirdDistrict: allDistrictOptions.filter(
					i =>
						!i.id ||
						!(
							i.id === fieldValue.primaryDistrict ||
			...
			switch (key) {
				case 'registrationNumber':
					if (!value) {
						...
						value &&
						!/^[a-zA-Z]{2}[0-9]{1,2}[a-zA-Z]{1,2}[0-9]{1,4}$/.test(value)
					) {
						err[key] =
							'Please Enter the vehicle number without spaces, eg: KL13AB1234';
					}
					break;
								...
								<Box display='flex' flexDirection='column'>
									...
									<div className={`nativeSelectMod ${classes.selectField}`}>
										<NativeSelectField
											...
											label='Vehicle Type'
										/>
										<ErrorHelperText error={errors.vehicleType} />
									</div>
									...
									<div className={`nativeSelectMod ${classes.selectField}`}>
										...
										/>
										<ErrorHelperText error={errors.insuranceValidTill} />
									</div>
									<TextInputField
										label='Name of owner'
										name='nameOfOwner'
									...
									<Box>
										<Typography>Select Your Ambulance Features</Typography>
									</Box>
									<Box
										display='flex'
										flexDirection='row'
