...
import { getLocalbodyByDistrict, getLocalbodyByName } from '../../../Redux/actions';
...
function LocalBodySelect(props: LocalBodySelectProps) {
    const { name, errors, className, multiple, selected, searchAll, setSelected, margin } = props
    ...
    const onLocalBodySearch = useCallback(debounce(async (text: string) => {
        if (text) {
    ...
    return (<AutoCompleteAsyncField
        ...
        noOptionsText={hasSearchText ? "No local body found, please try again" : "Start typing to begin search"}
