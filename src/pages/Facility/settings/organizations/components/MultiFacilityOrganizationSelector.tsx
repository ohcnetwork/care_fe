...
export default function FacilityOrganizationSelector(
  ...
  const handleLevelChange = (values: string[], level: number) => {
    ...
    // Remove parent if a child is selected
    const newLevels = selectedLevels.slice(0, level).filter(
      (org) => !selectedOrgs.some((child) => child.id === org.id), // Remove parent if its child is in selectedOrgs
    );
