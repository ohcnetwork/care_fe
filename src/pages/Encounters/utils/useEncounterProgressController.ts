...
const getStartEncounterRequests = (
  {
    encounter,
    appointment,
  }: {
  ...
  firstServicePointId: string | null,
): BatchRequestBody<
  AppointmentUpdateRequest | TokenUpdate | EncounterEdit
>["requests"] => {
  ...
  if (appointment) {
    requests.push({
      url: scheduleApi.appointments.update.path
        .replace("{facilityId}", appointment.facility.id)
  ...
  if (appointment?.token) {
    requests.push({
      url: tokenApi.update.path
        .replace("{facility_id}", appointment.facility.id)
...
export function useEncounterProgressController(
  appointment?: AppointmentRead,
): EncounterProgressControllerReturnType {
  ...
  const { data: servicePoints } = useQuery({
    queryKey: ["servicePoints", appointment?.facility?.id],
    queryFn: query(tokenSubQueueApi.list, {
      pathParams: { facility_id: appointment?.facility?.id ?? "" },
      queryParams: {
        resource_type: appointment?.resource_type,
        resource_id: appointment?.resource?.id,
        status: TokenSubQueueStatus.ACTIVE,
      },
    }),
    enabled: !!appointment && !!appointment.facility?.id,
  });
  ...
  return {
    ...
    startEncounter: ({
      ...
      const requests = getStartEncounterRequests(
        { encounter, appointment },
        servicePoints?.results[0]?.id ?? null,
      );
