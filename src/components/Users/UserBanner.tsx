...
export default function UserBanner({ userData }: { userData: UserModel }) {
  ...
  useEffect(() => {
    if (!userData) return;
    setUserOnline(isUserOnline(userData));
  }, [userData]);
                ...
                <div className="flex flex-col self-center">
                  <div className="flex flex-row items-center gap-3">
                    ...
                    <div className="min-width-50 shrink-0 text-sm text-secondary-600">
                      {userData.last_login && userOnline ? (
                        <span>
                          {" "}
                        ...
                        <>
                          <div className="rounded-full bg-primary-100 px-3 py-px">
                            <span
                              aria-label="Online"
                            ...
                            ></span>
                            <span className="pl-2 text-xs text-primary-600">
                              {userData.last_login
                                ? relativeTime(userData.last_login)
                                : "Never"}
                            </span>
                          </div>
                        </>
