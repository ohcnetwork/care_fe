...
interface MedicationPrescriptionTabProps {
  patientId?: string;
  encounterId?: string;
  canAccess?: boolean;
  canWrite?: boolean;
  facilityIdExists?: boolean;
}

export function MedicationPrescriptionTab({
  patientId,
  encounterId,
  canAccess = true,
  canWrite = false,
  facilityIdExists = false,
}: MedicationPrescriptionTabProps) {
  const { t } = useTranslation();
      ...
      {isLoading ? (
        ...
        <ScrollArea className="h-fit">
          <div className="min-w-[800px]">
            ...
            {!!stoppedMedications?.results?.length && !searchQuery.trim() && (
              <div
