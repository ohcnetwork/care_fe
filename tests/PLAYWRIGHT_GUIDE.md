...
7. **Using custom timeouts / manual waits** — Avoid hardcoded `{ timeout: 15000 }` (Playwright guidance; not lint-enforced), `page.waitForTimeout()`, and `page.waitForLoadState("networkidle")` (both enforced as errors). Instead use web-first assertions like `expect(locator).toBeVisible()` to wait for page readiness. Rely on global timeouts from `playwright.config.ts`.
