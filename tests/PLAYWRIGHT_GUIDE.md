...
7. **Using custom timeouts / manual waits** — Avoid hardcoded `{ timeout: 15000 }` (Playwright guidance; not lint-enforced), `page.waitForTimeout()` (enforced as an error), and `page.waitForLoadState("networkidle")` (currently a warning, being migrated to an error). Instead use web-first assertions like `expect(locator).toBeVisible()` to wait for page readiness. Rely on global timeouts from `playwright.config.ts`.
