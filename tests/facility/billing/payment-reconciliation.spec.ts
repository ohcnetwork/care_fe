...
test.describe("Payment Reconciliation", () => {
  ...
  test("should record payment with required fields", async ({ page }) => {
    // Open Record Payment
  ...
  test("should record payment with all fields filled", async ({ page }) => {
    ...
    const locationOptions = await page
      .locator('[data-slot="command-item"]')
      .all();
    const availableLocations = locationOptions.slice(
      0,
      Math.min(5, locationOptions.length),
    );
    const randomLocation = faker.helpers.arrayElement(availableLocations);
  ...
  test("should persist selected location when reopening payment form", async ({
    page,
  }) => {
    ...
    const firstOption = await page
      .locator('[data-slot="command-item"]')
      .first();
    const selectedLocationName = await firstOption.textContent();
    await firstOption.click();

    // Close the dialog (ESC key)
    await page.keyboard.press("Escape");

    // Reopen the payment form
    await page.getByRole("button", { name: /record payment/i }).click();

    // Verify the location is still selected (button shows location name, not "Select Location")
    if (selectedLocationName) {
      await expect(
        page
          .getByRole("combobox")
          .filter({ hasText: selectedLocationName.trim() }),
      ).toBeVisible();
    }
  });
});
