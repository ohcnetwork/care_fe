...
async function createPatient(): Promise<string> {
  ...
  // Backend occasionally returns 400 "Patient creation failed, try again after
  // a while" — retry with a fresh phone number to make setup resilient.
  let lastError = "";
  for (let attempt = 0; attempt < 3; attempt++) {
    const attemptPhone =
      attempt === 0
        ? phone
        : `+91${faker.helpers.fromRegExp(/[6-9][0-9]{9}/)}`;
