...
test.describe("Enable When Functionality Tests", () => {
  ...
  test.describe("Integer 'equals' operator", () => {
    /**
     * Tests the 'equals' operator for integer fields
     * Coverage:
     * - Operator: equals (==)
     * - Type: integer
     * - Scenario: Perfect score message appears only when score is exactly 100
     * - Validation: Required field shown only for exact match
     */
    test("should show perfect score notes for score of 100", async ({
      page,
    }) => {
      const perfectScore = "100";
      const scoreNotes = "Excellent performance!";

      await checkVisibility(page, "🎉 Perfect Score - Additional Notes", false);

      await fillFormField(page, "Risk Score", "input", "99");
      await checkVisibility(page, "🎉 Perfect Score - Additional Notes", false);

      await fillFormField(page, "Risk Score", "input", perfectScore);
      await checkVisibility(page, "🎉 Perfect Score - Additional Notes", true);
      await fillFormField(
        page,
        "🎉 Perfect Score - Additional Notes",
        "input",
        scoreNotes,
      );
    ...
    /**
     * Tests validation for conditionally shown required field
     * Coverage:
     * - equals operator requires field when condition is met
     * - Validation error shown for unfilled required field
     */
    test("should require notes when score is 100", async ({ page }) => {
      await fillFormField(page, "Risk Score", "input", "100");
      await checkVisibility(page, "🎉 Perfect Score - Additional Notes", true);

      await fillBaseFields(page);
      await page.getByRole("button", { name: /submit|save/i }).click();

      await expectQuestionnaireFieldError(
        page,
        "🎉 Perfect Score - Additional Notes",
      );
    });
  ...
  test.describe("Integer 'not_equals' operator", () => {
    /**
     * Tests the 'not_equals' operator for integer fields
     * Coverage:
     * - Operator: not_equals (!=)
     * - Type: integer
     * - Scenario: Explanation field appears when dosage is not standard (100mg)
     * - Validation: Field hidden when value matches exclusion condition
     */
    test("should show explanation for non-standard dosage", async ({
      page,
    }) => {
      const dosageAmount = "50";
      const dosageExplanation = "Reduced due to patient weight";

      await checkVisibility(page, "Non-standard Dosage Explanation", false);

      await fillFormField(page, "Dosage Amount (mg)", "input", "100");
      await checkVisibility(page, "Non-standard Dosage Explanation", false);

      await fillFormField(page, "Dosage Amount (mg)", "input", dosageAmount);
      await checkVisibility(page, "Non-standard Dosage Explanation", true);
      await fillFormField(
        page,
        "Non-standard Dosage Explanation",
        "input",
        dosageExplanation,
      );
    ...
    /**
     * Tests that field is hidden for standard dosage
     * Coverage:
     * - not_equals operator correctly hides field when condition is false
     * - Submission succeeds without filling hidden required field
     */
    test("should not require explanation for standard dosage", async ({
      page,
    }) => {
      await fillFormField(page, "Dosage Amount (mg)", "input", "100");
      await checkVisibility(page, "Non-standard Dosage Explanation", false);
  ...
  test.describe("Medication 'exists' operator", () => {
    /**
     * Tests 'exists' operator for medication field
     * Coverage:
     * - Operator: exists (checks if field has any value)
     * - Type: string
     * - Scenario: Review field appears when medications are entered
     * - Dynamic behavior: Field disappears when medication is cleared
     */
    test("should toggle medication review visibility", async ({ page }) => {
      const reviewField = "Medication Review Required";
      await checkVisibility(page, reviewField, false);
      ...
      await checkVisibility(page, reviewField, true);
